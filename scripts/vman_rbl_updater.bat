@echo off
echo.
set VERSION=** VMAN Retro-Bliss LevelUp! Updater v1.2 by Bilu **
IF EXIST %TEMP%\vman_update.tmp (
echo %VERSION%
echo.
echo Starting updates, logs available at V:\_tools 
powershell -command "V:\_tools\vman-retrobat-master\apply_updates.bat 2>&1 | tee ('V:\_tools\vman-update-{0}.log' -f (Get-Date -format 'yyyyMMddHHmm'))"
) ELSE (
cls
echo %VERSION%
echo.
